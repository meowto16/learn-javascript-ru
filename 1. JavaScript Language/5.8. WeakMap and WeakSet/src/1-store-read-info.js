/**
 * Хранение отметок "не прочитано" (важность: 5)
 ...
 * Имея такую вводную информацию, решите, какую структуру данных вы могли бы использовать для ответа на
 * вопрос «было ли сообщение прочитано?». Структура должна быть подходящей, чтобы можно было однозначно сказать, было ли прочитано это сообщение для каждого объекта сообщения.
...
const MessagesReadStatusBase = new function () {
  ...
  this.readMessage = function (message) {
    return this.set.add(message)
  }
  this.isMessageRead = function (message) {
    return this.set.has(message)
  }
}
